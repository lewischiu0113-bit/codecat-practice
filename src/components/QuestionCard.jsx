...
const QuestionCard = ({ question, index, userAnswer, onAnswerChange, showResult = false, isCorrect: externalIsCorrect = null }) => {
  ...
  return (
    <div
      className={`bg-white rounded-xl shadow-sm border-2 p-4 sm:p-6 mb-4 sm:mb-6 transition-all duration-300 ${
        isCorrect 
    ...
    >
      <div className="flex items-start gap-2.5 sm:gap-3 mb-3 sm:mb-4">
        <div
          className={`flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center font-semibold text-xs sm:text-sm transition-all duration-300 ${
            isCorrect
        ...
        <div className="flex-1 min-w-0">
          <div className="text-base sm:text-lg font-semibold text-gray-800 mb-3 break-words max-w-full overflow-x-auto">
            <ReactMarkdown
              components={{
                  ...
                  return !inline && match ? (
                    <div className="my-3 overflow-x-auto max-w-full">
                      <SyntaxHighlighter
                        ...
                        language={match[1]}
                        PreTag="div"
                        className="rounded-lg border border-gray-200 shadow-sm text-xs sm:text-sm"
                        customStyle={{
                          margin: 0,
                          borderRadius: '0.5rem',
                        }}
                    ...
                    <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs sm:text-sm font-mono text-primary break-all" {...props}>
                      {children}
                    </code>
                  );
                },
              ...
              {question.options.map((option, optIndex) => {
                ...
                return (
                  <button
                    ...
                    disabled={showResult}
                    className={`w-full text-left px-3 py-2.5 sm:px-4 sm:py-3 rounded-lg border-2 transition-all duration-300 transform text-sm sm:text-base ${
                      isSelected
                        ? isCorrect
                          ? 'border-green-500 bg-green-100 scale-102 shadow-md'
                          : isWrong
                          ? 'border-red-500 bg-red-100 scale-102 shadow-md'
                          : 'border-primary bg-orange-50 scale-102 shadow-md'
                        : isCorrectOption
                        ? 'border-green-300 bg-green-50'
                        : 'border-gray-200 hover:border-primary hover:bg-orange-50 hover:scale-[1.01] hover:shadow-sm'
                    } ${showResult ? 'cursor-default' : 'cursor-pointer active:scale-95'}`}
                  >
                    <span className="font-medium text-gray-800 break-words">{option}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div className="space-y-3">
              <div className="relative">
                ...
                <textarea
                  ...
                  placeholder="輸入格式代碼..."
                  rows={6}
                  className={`w-full pl-9 pr-3 sm:pl-10 sm:pr-4 py-2.5 sm:py-3 border-2 rounded-lg font-mono text-xs sm:text-sm resize-y min-h-[120px] transition-all duration-300 ${
                    isCorrect
                      ? 'border-green-500 bg-green-50 animate-scale-in'
                      : isWrong
                      ? 'border-red-500 bg-red-50 animate-shake'
              ...
              {showResult && (
                <div className="text-xs sm:text-sm text-gray-600">
                  ...
                  <code className="ml-2 px-2 py-1 bg-gray-100 rounded font-mono text-primary whitespace-pre-line block mt-1 overflow-x-auto text-xs sm:text-sm">
          ...
          {question.type === 'CodeExecution' && (
            <div className="space-y-3">
              ...
              {question.testCases && question.testCases.length > 0 && (
                <div className="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200 max-w-full overflow-x-auto">
                  <div className="text-xs sm:text-sm font-semibold text-gray-700 mb-2">測試案例：</div>
                  {question.testCases.map((testCase, idx) => (
                    <div key={idx} className="mb-3 last:mb-0">
                      ...
                      <div className="bg-white rounded p-2 border border-gray-200">
                        ...
                        <pre className="text-xs font-mono text-gray-800 whitespace-pre-wrap break-all">{testCase.input || '(無輸入)'}</pre>
                        <div className="text-xs text-gray-500 mb-1 mt-2">預期輸出：</div>
                        <pre className="text-xs font-mono text-gray-800 whitespace-pre-wrap break-all">{testCase.expectedOutput}</pre>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="relative">
                ...
                <textarea
                  ...
                  rows={10}
                  className={`w-full pl-9 pr-3 sm:pl-10 sm:pr-4 py-2.5 sm:py-3 border-2 rounded-lg font-mono text-xs sm:text-sm resize-y min-h-[160px] sm:min-h-[200px] transition-all duration-300 ${
                    isCorrect
                      ? 'border-green-500 bg-green-50 animate-scale-in'
                      : isWrong
                      ? 'border-red-500 bg-red-50 animate-shake'
              ...
              {!showResult && (
                <button
                  ...
                  className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-105 active:scale-95 text-xs sm:text-sm font-medium"
                >
                  {isRunning ? (
                    <>
